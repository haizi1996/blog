...
import com.hailin.blog.dao.es.EsBlogRepository;
...
public class EsBlogServiceImpl implements EsBlogService {
    ...
    public Page<EsBlog> listNewestEsBlogs(String keyword, Pageable pageable) throws SearchParseException {
        ...
        pages = EMPTY_KEYWORD.equals(keyword)? esBlogRepository.findAll(pageable) : esBlogRepository.findByTitleOrSummaryOrContentOrTags(
    ...
    public Page<EsBlog> listHotestEsBlogs(String keyword, Pageable pageable) throws SearchParseException {
        ...
        return EMPTY_KEYWORD.equals(keyword)? esBlogRepository.findAll(pageable) :esBlogRepository.findByTitleOrSummaryOrContentOrTags(
                keyword, keyword, keyword, keyword, pageable);
    }
    ...
    public List<User> listTop12Users() {
        ...
        List<StringTerms.Bucket> modelBucketIt =  modelTerms.getBuckets();
        if(!CollectionUtils.isEmpty(modelBucketIt)){
            usernamelist = modelBucketIt.stream().map(actiontypeBucket->actiontypeBucket.getKeyAsString()).collect(Collectors.toList());Collectors.toList();
        }
//        for (StringTerms.Bucket actiontypeBucket : modelBucketIt) {
